...
constexpr auto IMM8_MAX = 0xFF;
constexpr auto IMM8_MIN = -0x100;
inline bool IS_IMM8(int x) { return x <= IMM8_MAX && x >= IMM8_MIN; }

constexpr auto IMM9_MAX = 0x1FF;
constexpr auto IMM9_MIN = -0x200;
inline bool IS_IMM9(int x) { return x <= IMM9_MAX && x >= IMM9_MIN; }

constexpr auto IMM12_MAX = 0x7FF;
constexpr auto IMM12_MIN = -0x800;
inline bool IS_IMM12(int x) { return x <= IMM12_MAX && x >= IMM12_MIN; }

constexpr auto IMM32_MAX = 0x7FFFFFFF;
constexpr auto IMM32_MIN = -0x80000000LL;
inline bool IS_IMM32(int64_t x) { return x <= IMM32_MAX && x >= IMM32_MIN; }

#define NUMBYTES(x) (((x)+7)>>3)
constexpr auto PROLOGUE_OFFSET_BASE = 16;
constexpr auto PROLOGUE_ALIGN = 16;
...
class ARMCodeGen
{
	                 ...
	                 ASMInstruction::InstType ty = ASMInstruction::Instruction)
	{
		...
		for (const auto& arg : args)
			content += arg + ", ";
		if (!args.size()) content.pop_back();
		else content.pop_back(), content.pop_back();
