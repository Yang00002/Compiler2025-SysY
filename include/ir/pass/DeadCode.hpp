...
{
public:
	DeadCode(const DeadCode& other) = delete;
	DeadCode(DeadCode&& other) noexcept = delete;
	DeadCode& operator=(const DeadCode& other) = delete;
	DeadCode& operator=(DeadCode&& other) noexcept = delete;
...
public:
	explicit DeadCode(PassManager* mng, Module* m) : Pass(mng, m), func_info(nullptr), work_list(new WorkList{})
	{
	}
...
private:
	...
	bool clear_basic_blocks(Function* func) const;
	// 消除函数用不到的参数
	bool clear_parameters(Function* func) const;
	// 删除无用的 alloca
	// 无用的 alloca 不存在任何的 load, 其也不作为函数参数
	bool clear_not_use_allocas(Function* func) const;
	// 删除无用的指令(只保留关键指令)
	bool clear_instructions(Function* func) const;
