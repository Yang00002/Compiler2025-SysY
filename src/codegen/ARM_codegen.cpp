...
void ARMCodeGen::Rload_stack_to_GPreg(Value* val, unsigned Rid)
{
    ...
    auto width = val->get_type()->sizeInBitsInArm64();
    MemAccess m = { .base = "X29", .offset = "#"+std::to_string(stack_offset) };
...
void ARMCodeGen::Rload_imm_to_GPreg(int64_t imm, unsigned Rid, unsigned bits)
{
    ...
    } else assert(false && "Invalid int width!\n");
}
...
void ARMCodeGen::Rload_64_to_GPreg(int64_t imm, unsigned Rid)
{
    ...
    for (int i = 0; i < 4; ++i) {
        if (parts[i]) {
            if (first) {
                append_inst("MOVZ", {"X" + std::to_string(Rid), "#" + std::to_string(parts[i]), 
                    "LSL #" + std::to_string(i<<4)});
                first = false;
            } else {
                append_inst("MOVK", {"X" + std::to_string(Rid), "#" + std::to_string(parts[i]), 
                    "LSL #" + std::to_string(i<<4)});
            }
        }
    }
...
void ARMCodeGen::Rload_imm_to_FPreg(float imm, unsigned FRid)
{
    ...
    append_inst("FMOV",{fvWidthPrefix.at(width)+std::to_string(FRid),
...
void ARMCodeGen::allocate()     
{
    ...
    for (auto& arg : context.func->get_args()) {
        if ( arg.get_type()==Types::FLOAT && farg_count < 8 ) { // 从寄存器读参数到栈上
            offset += NUMBYTES(arg.get_type()->sizeInBitsInArm64());
            context.offset_map[&arg] = -offset;
            ++farg_count;
        } else if ( iarg_count < 8 ){  // 从寄存器读参数到栈上
            offset += arg.get_type()==Types::BOOL ? NUMBYTES(Types::INT->sizeInBitsInArm64()) :
            ...
            arg_offset += arg.get_type()==Types::BOOL ? NUMBYTES(Types::INT->sizeInBitsInArm64()) :
                          NUMBYTES(arg.get_type()->sizeInBitsInArm64());
        }
    }
...
void ARMCodeGen::gen_binary()
{
    ...
    switch (bin_inst->get_instr_type()) {
        case Instruction::add: 
            append_inst("ADD", {p+std::to_string(res_reg_id), 
                p+std::to_string(op1_reg_id), p+std::to_string(op2_reg_id)}); 
            break;
        case Instruction::sub: 
            append_inst("SUB", {p+std::to_string(res_reg_id), 
                p+std::to_string(op1_reg_id), p+std::to_string(op2_reg_id)}); 
            break;
        case Instruction::mul: 
            append_inst("MUL", {p+std::to_string(res_reg_id), 
                p+std::to_string(op1_reg_id), p+std::to_string(op2_reg_id)}); 
            break;
        case Instruction::sdiv: 
            append_inst("SDIV", {p+std::to_string(res_reg_id), 
                p+std::to_string(op1_reg_id), p+std::to_string(op2_reg_id)}); 
            break;
        case Instruction::srem:
            append_inst("SDIV", {p+std::to_string(res_reg_id), 
                    p+std::to_string(op1_reg_id), p+std::to_string(op2_reg_id)}); 
            append_inst("MSUB", {p+std::to_string(res_reg_id), p+std::to_string(op2_reg_id), 
...
void ARMCodeGen::gen_fbinary()
{
    ...
    switch (bin_inst->get_instr_type()) {
        case Instruction::fadd: 
            append_inst("FADD", {p+std::to_string(res_freg_id), 
                p+std::to_string(op1_freg_id), p+std::to_string(op2_freg_id)}); 
            break;
        case Instruction::fsub: 
            append_inst("FSUB", {p+std::to_string(res_freg_id), 
                p+std::to_string(op1_freg_id), p+std::to_string(op2_freg_id)});
            break;
        case Instruction::fmul: 
            append_inst("FMUL", {p+std::to_string(res_freg_id), 
                p+std::to_string(op1_freg_id), p+std::to_string(op2_freg_id)}); 
            break;
        case Instruction::fdiv: 
            append_inst("FDIV", {p+std::to_string(res_freg_id), 
                p+std::to_string(op1_freg_id), p+std::to_string(op2_freg_id)}); 
            break;
...
void ARMCodeGen::gen_load()
{
    ...
    if ( load_inst->get_type()==Types::FLOAT ) {
        ...
        append_inst("LDR", fp+std::to_string(ld_freg_id), MemAccess{.base = "X"+std::to_string(ld_reg_id)});
        Rstore_from_FPreg(load_inst, ld_freg_id);
    } else {
        auto p = iWidthPrefix.at(load_inst->get_type()->sizeInBitsInArm64());
        append_inst("LDR", p+std::to_string(ld_reg_id), MemAccess{.base = "X"+std::to_string(ld_reg_id)});
...
void ARMCodeGen::gen_store()
{
    ...
    if ( val->get_type()==Types::FLOAT ) {
        ...
        append_inst("STR", p+std::to_string(st_freg_id), MemAccess{.base = "X"+std::to_string(st_adr_reg_id)});
    } else {
        auto st_reg_id = 9;
        Rload_to_GPreg(val, st_reg_id);
        auto p = iWidthPrefix.at(val->get_type()->sizeInBitsInArm64());
        append_inst("STR", p+std::to_string(st_reg_id), MemAccess{.base = "X"+std::to_string(st_adr_reg_id)});
    }
}
...
void ARMCodeGen::gen_icmp()
{
    ...
    switch (icmp_inst->get_instr_type()) {
        case Instruction::eq: cond = "EQ"; break;
        case Instruction::ne: cond = "NE"; break;
        case Instruction::gt: cond = "GT"; break;
        case Instruction::ge: cond = "GE"; break;
        case Instruction::lt: cond = "LT"; break;
        case Instruction::le: cond = "LE"; break;
...
void ARMCodeGen::gen_fcmp()
{
    ...
    switch (fcmp_inst->get_instr_type()) {
        case Instruction::feq: cond = "EQ"; break;
        case Instruction::fne: cond = "NE"; break;
        case Instruction::fgt: cond = "GT"; break;
        case Instruction::fge: cond = "GE"; break;
        case Instruction::flt: cond = "MI"; break;
        case Instruction::fle: cond = "LS"; break;
...
void ARMCodeGen::gen_gep()
{
        ...
        if (auto const_int = dynamic_cast<ConstantValue*>(index_val)) {
            ...
            if (offset != 0) {
                ...
                if (total_offset != 0){
                    ...
                    append_inst("ADD", {"X"+std::to_string(offset_reg_id), 
            ...
            if ( type_size != 0 ) {
                ...
                append_inst("MUL", {"X"+std::to_string(tmp_reg_id), 
    ...
    append_inst("ADD", {"X"+std::to_string(base_reg_id), "X"+std::to_string(base_reg_id),
...
void ARMCodeGen::gen_call()
{
    ...
    for (int i=1; i < call_inst->get_num_operand(); ++i) {
        ...
        if ( arg->get_type()==Types::FLOAT && Nfarg < 8)
            ...
            total_param_size += arg->get_type()==Types::BOOL ? NUMBYTES(Types::INT->sizeInBitsInArm64()) :
                                NUMBYTES(arg->get_type()->sizeInBitsInArm64());
        }
    }
    ...
    if(total_param_size > 0){
        ...
        for(auto p = stack_params.rbegin(); p != stack_params.rend(); ++p){
            auto arg = *p;
            auto sz = arg->get_type()==Types::BOOL  ? Types::INT->sizeInBitsInArm64() :
            ...
            if(arg->get_type()==Types::FLOAT){
                ...
                MemAccess{.base = "SP", .offset = "#"+std::to_string(arg_offset)});
            } else {
...
void ARMCodeGen::gen_prologue()
{
    ...
    for (auto& arg : context.func->get_args()) { // 加载不在 caller 栈帧上的参数
        if ( arg.get_type()==Types::FLOAT && flt_id < 8 ) { 
            MemAccess m = { .base = "X29", .offset = "#"+std::to_string(context.offset_map.at(&arg)) };
