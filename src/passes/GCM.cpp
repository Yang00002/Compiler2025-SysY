...
bool GlobalCodeMotion::is_pinned(Instruction* i) 
{ 
    return i->is_br() || i->is_call() || i->is_ret() || i->is_phi() || i->is_load() || i->is_store(); 
}

BasicBlock* GlobalCodeMotion::LCA(BasicBlock* bb1, BasicBlock* bb2)
{
    if (bb1 == bb2) return bb1;
    if (dom_->is_dominate(bb1, bb2)) return bb1;
    if (dom_->is_dominate(bb2, bb1)) return bb2;
        ...
        if( i != inst ){
            if ( !inst->is_phi() ) {
                ...
                if( std::find_if(phi_use.begin(),phi_use.end(),
                    ...
                    i->get_parent()->erase_instr(i);
                    auto& insts = best->get_instructions();
