...
bool GVN::is_commutative(Instruction* i)
{
...
std::string GVN::get_expr(Instruction* i)
{
    ...
    } else if(i->is_si2fp()||i->is_fp2si()||i->is_zext()||i->is_nump2charp()||
        i->get_instr_type() == Instruction::global_fix){
        auto op = i->get_instr_op_name();
        auto opnd0 = i->get_operand(0)->print();
        return op+","+opnd0;
...
// 交换操作数顺序不改变结果的指令
std::string GVN::get_equiv_expr(Instruction* i)
{
